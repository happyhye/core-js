...
let greaterThen = number[0] > number[3]; //false
console.log(greaterThen);

// ~ 보다 크거나 작음 또는 같음 → boolean
let lessThenAndEqual = number[3] <= number[1]; //false
let greaterThenAndEqual = number[0] >= number[1]; //ture

// 동등(등급이나 정도가 같다) → boolean
let equal = number[0] == number[1];

// 다름(같지 않음) → boolean
let unequal = number[0] != number[1]; //false
console.log(unequal);
